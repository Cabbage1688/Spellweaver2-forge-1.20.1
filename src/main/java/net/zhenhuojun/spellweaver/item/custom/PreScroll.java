...
public abstract class PreScroll extends Item {
    protected final double maxMana;

    public PreScroll(Properties properties, double maxMana) {
        super(properties);
    ...
    public boolean isBarVisible(ItemStack stack) {
        ...
        return tag != null && tag.contains("mana") && tag.getDouble("mana") >= 0 && tag.getDouble("mana") < maxMana;
    }
    ...
    public int getBarWidth(ItemStack stack) {
        ...
        return Math.round((float) (mana / maxMana) * 13.0F);
    }
    ...
    public void onUseTick(Level level, LivingEntity entity, ItemStack stack, int remainingUseDuration) {
        ...
        if (player.isCrouching()) {
            ...
            if (tag.contains("mana")) {
                ...
                if (manaInItem < maxMana) {
                    double maxExtract = Math.min(8, ManaUtil.CheckMana(player));
                    double availableSpace = maxMana - manaInItem;
